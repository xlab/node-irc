...
Client.prototype.connect = function(retryCount, callback) {
    ...
    function handleData(chunk) {
        ...
        var lines = self.convertEncoding(buffer, msgInbound).toString().split(lineDelimiter);
...
Client.prototype._speak = function(kind, target, text) {
    ...
    if (typeof text !== 'undefined') {
        var encodedText = self.convertEncoding(text, msgOutbound)
        encodedText.toString().split(/\r?\n/).filter(function(line) {
            return line.length > 0;
        }).forEach(function(line) {
...
Client.prototype.convertEncoding = function(str, direction) {
    var self = this, out = str;

    if (self.opt.encoding && direction > 0) {
        try {
            var Iconv = require('iconv').Iconv;
            if (direction == msgInbound) {
                var converter = new Iconv(self.opt.encoding, 'utf-8');
                out = converter.convert(str);
            } else if (direction == msgOutbound) {
                var converter = new Iconv('utf-8', self.opt.encoding);
                out = converter.convert(str);
            }
        } catch (err) {
            if (self.opt.debug) {
